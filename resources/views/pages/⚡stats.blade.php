        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="overflow-x-auto overflow-y-hidden">
                <div class="flex h-48 items-end gap-1">
                    @foreach ($months as $m)
                        <div class="flex w-10 shrink-0 flex-col items-center gap-1" title="{{ $m['label'] }}: {{ $it($m['count']) }}">
                            <div class="w-full rounded-t bg-accent transition-all"
                                style="height: {{ max(2, (int) round($m['count'] / $maxMonth * 160)) }}px"></div>
                            <flux:text class="whitespace-nowrap text-[10px] text-zinc-500 dark:text-zinc-400">{{ $m['label'] }}</flux:text>
                        </div>
                    @endforeach
                </div>
            </div>
